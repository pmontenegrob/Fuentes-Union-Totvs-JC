...
//	begin transaction
	...
	if empty(cBody)
		...
		if Type("oObj:C5_UOBSERV") <> "U"// caso exista
			...
			aadd(aCabec,{"C5_UOBSERV","Venta M�vil",Nil})
		endif
		if Type("oObj:DDATABASE") <> "U"// caso exista
		...
		/**
		* DT 17/06/2022
		* Union solicita que la moneda para el t�tulo sea siempre en D�lares
		**/
		...
		if Type("oObj:C5_UTIPOEN") <> "U"// en caso tenga tipo de entrega se incluye Nahim 07/06/2020 Especifico Uni�n Agronegocios.
			aadd(aCabec,{"C5_UTIPOEN",cvaltochar(oObj:C5_UTIPOEN),Nil})
		endif

		if Type("oObj:C5_CODMPAG") <> "U"//Facturaci�n electr�nica - C�digo del M�todo de Pago
			aadd(aCabec,{"C5_CODMPAG",cvaltochar(oObj:C5_CODMPAG),Nil})
		endif

		if Type("oObj:C5_XCLDOCI") <> "U"//Facturaci�n electr�nica - Complemento de Documento de Identidad
			aadd(aCabec,{"C5_XCLDOCI",cvaltochar(oObj:C5_XCLDOCI),Nil})
		endif

		if Type("oObj:C5_XTIPDOC") <> "U"//Facturaci�n electr�nica - Tipo Documento de Identidad
			aadd(aCabec,{"C5_XTIPDOC",cvaltochar(oObj:C5_XTIPDOC),Nil})
		endif

		if Type("oObj:C5_XEMAIL") <> "U"//Facturaci�n electr�nica - Email
			aadd(aCabec,{"C5_XEMAIL",cvaltochar(oObj:C5_XEMAIL),Nil})
		endif

		if Type("oObj:C5_NUMTARG") <> "U"//Facturaci�n electr�nica - N�mero Tarjeta
...
//							aadd(aCabec,{"C5_DOCGER",'2',Nil}) // adicionando local
			...
			// Justificaci�n de descuento C6_UTPLIQ
			if Type("oObj:productos[nX]:C6_UTPLIQ") <> "U"// caso exista
				aadd(aLinha,{"C6_UTPLIQ",oObj:productos[nX]:C6_UTPLIQ,Nil}) // justificaci�n de descuento por linea. Nahim 07/06/2020
			endif
...
WSMETHOD GET WSRECEIVE CODVEN WSSERVICE pedido // recibir por QueryString si hay codigo de usuario para filtrar
	...
	If (cNextAlias)->( !Eof() )
		...
		While !(cNextAlias)->(Eof() )
			...
			IF alltrim(oPedido['C5_NUM']) != alltrim((cNextAlias)->C5_NUM)
				...
				oPedido['C5_UOBSERV']	:= (cNextAlias)->C5_UOBSERV // agregando observaci�n Nahim 04/02/2020
				...
				oPedido['token']		:= (cNextAlias)->C5_UIDAPP // id App m�vil (TOKEN)
				...
				if !empty((cNextAlias)->COLOR) // caso que tenga color
					if (alltrim((cNextAlias)->C5_COLOR)) == '#ebeb5f' // s�lo si es amarillo
	...
	objProducto['C6_UTPLIQ']	:= (cNextAlias)->C6_UTPLIQ// descuento justificaci�n Nahim 07/06/2020
	...
	//			objProducto['C6_VDOBS']    := (cNextAlias)->C6_VDOBS)
			...
			IF (alltrim(oPedido['C5_NUM']) != alltrim((cNextAlias)->C5_NUM)) .or. (cNextAlias)->(Eof()) // caso est� cambiando de pedido vamos a v
