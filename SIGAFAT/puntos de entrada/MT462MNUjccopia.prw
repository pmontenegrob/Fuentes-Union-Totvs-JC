...
User Function MT462MNU()
	...
	Do Case
		...
		Case FUNNAME() == 'MATA467N'
			...
			AADD(AROTINA,{ 'Imprimir Facturas para fiscalizacion ',"u_imprFat2()"     , 0 , 9})
...
user FUNCTION imprFat()
	...
	elseif( val(nlote) == 2)
		...
		if (nEnLinea <> "1")
			...
			//cDirUsr := GetTempPath()
			cDirUsr := "C:\Users\jcampos\Desktop\facturas\"

			//D:\TOTVS12\MP12PRD\protheus_data\system\cfd\facturas
			cDirSrv := GetSrvProfString('startpath','')+'\cfd\facturas\'
			//cDirSrv2 := "C:\Users\jcampos\Desktop\facturas\"
...
user FUNCTION imprFat2()
	...
	While (cTemp)->(!EOF())
		...
		(cTemp)->(dbSkip())
	enddo
	//MSGINFO("ENTRANDOOO" , "AVISO:"  )

	cQryAux := " SELECT "                                                                 + CRLF
	cQryAux += "     F2_FILIAL, F2_SERIE, F2_DOC "                                        + CRLF
	cQryAux += " FROM "                                                                   + CRLF
	cQryAux += "     " + RetSQLName('SF2') + " SF2 "                                      + CRLF
	cQryAux += " WHERE "                                                                  + CRLF
	cQryAux += "     D_E_L_E_T_ = ' ' "                                                   + CRLF
	cQryAux += "     AND F2_ESPECIE = 'NF' "                                              + CRLF
	cQryAux += "     AND F2_EMISSAO BETWEEN '20211201' AND '20211231' "                   + CRLF
	cQryAux += "     AND R_E_C_N_O_ IN (SELECT RECNO FROM TMP_SF2_RECNO "                 + CRLF
	cQryAux += "                         WHERE ANIO_MES = '202112') "                     + CRLF
	cQryAux += " ORDER BY "                                                               + CRLF
	cQryAux += "     F2_FILIAL, F2_SERIE, F2_DOC "                                        + CRLF
	//MSGINFO("QUERY: "+cQryAux , "AVISO:"  )
    TCQuery cQryAux New Alias "QRY_SF2"
    //abril 10 se corto / enero 25 se corto(ccoriendo el 25, se volvio a correr)/16 de marzo se corto/ 19 de abril se corto
	...
	//MSGINFO("ENTRANDOOO2" + QRY_SF2->F2_FILIAL , "AVISO:"  )
    While ! QRY_SF2->(EoF())
		//MSGINFO("documento: " + QRY_SF2->F2_DOC + " serie: " + QRY_SF2->F2_SERIE + " FILIAL: " + QRY_SF2->F2_FILIAL, "AVISO:"  )
      
		U_FACTLOCAL(QRY_SF2->F2_DOC,QRY_SF2->F2_DOC,QRY_SF2->F2_SERIE,QRY_SF2->F2_FILIAL,1,"COPIA")
