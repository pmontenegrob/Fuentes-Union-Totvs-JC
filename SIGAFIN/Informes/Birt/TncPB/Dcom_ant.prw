...
User_Dataset Dcom_ant
title "REPORTE COMPARATIVO DE CONTROL PRESUPUESTARIO3 prueba"
...
BeginSql alias cTemp
	...
	SUM(CASE
               WHEN RANGO = '0' THEN CASE
                                         WHEN MONEDA = 2 THEN ROUND(SALDO * dbo.UC_FUN_TASA(%Exp:nfechaAnt%), 2)*SIGNO
	...
	FROM
	   (
		  SELECT
			 ...
			 SALDO, 
			 CASE
			   WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 0 THEN '0'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 0
                    AND DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 30 THEN '30'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 31
                    AND DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 60 THEN '60'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 61
                    AND DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 90 THEN '90'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 91
                    AND DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 120 THEN '120'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 121
                	AND DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 150 THEN '150'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 151
                	AND DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 180 THEN '180'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 181
                	AND DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) <= 365 THEN '365'
               WHEN DATEDIFF(dayofyear, E1.E1_VENCTO, %Exp:nfechaAnt%) > 365 THEN '366'
			 END
			 RANGO 
		  FROM %table:SE1% E1
		  JOIN %table:SA1% A1 
